...
contract Wormhole is ReentrancyGuard {
    ...
    // Address of the official WQTUM contract
    address constant WQTUMAddress = 0xE7E5CAAe57B34b93C57AF9478a5130f62E3D2827;
    ...
    function lockQTUM(
        ...
        uint256 transfer_amount = msg.value.mul(10 ** 1);
        require(transfer_amount != 0, "truncated amount must not be 0");

        // Wrap tx value in WQTUM
        WQTUM(WQTUMAddress).deposit{value : msg.value}();

        // Log deposit of WQTUM
        emit LogTokensLocked(target_chain, CHAIN_ID, 8, bytes32(uint256(WQTUMAddress)), bytes32(uint256(msg.sender)), recipient, transfer_amount, nonce);
    }

    fallback() external payable {revert("please use lockQTUM to transfer QTUM to Solana");}

    receive() external payable {revert("please use lockQTUM to transfer QTUM to Solana");}
}


interface WQTUM is IERC20 {
